...
async function getRecentContactsMap() {
  ...
  items.forEach((item, index) => {
    ...
    if (key) map.set(key, { index, ts: item.ts || 0 });
  });
  return map;
}
...
async function saveRecentContact(contact) {
  ...
  filtered.unshift({ name: contact.name || contact.email, email: contact.email, ts: Date.now() });
  await browser.storage.local.set({ [RECENT_CONTACTS_KEY]: filtered.slice(0, 20) });
}

async function registerComposeScripts() {
  if (composeScriptRegistered) return;
  try {
  ...
  function visitBook(book) {
    if (book.contacts && Array.isArray(book.contacts)) {
      for (const c of book.contacts) {
        ...
        if (primaryEmail) {
          contacts.push({ id: c.id, name: displayName.trim() || primaryEmail, email: primaryEmail.trim() });
        }
        if (secondEmail) {
          contacts.push({ id: `${c.id}-secondary`, name: displayName.trim() || secondEmail, email: secondEmail.trim() });
        }
      }
    }
    if (book.mailingLists && Array.isArray(book.mailingLists)) {
      for (const list of book.mailingLists) visitBook(list);
    }
  }

  for (const book of books) visitBook(book);
  return contacts;
}
...
async function searchContacts(query, tabId = null) {
  ...
  if (query) {
    ...
    if (q.includes("@") && q.includes(".") && !seen.has(q)) {
      unique.push({ id: `external-${q}`, name: q.split("@")[0], email: q, isExternal: true });
    }
    if (q.includes("@")) {
      const parts = q.split("@");
      const userPart = parts[0];
      if (userPart) {
        unique.unshift({ id: `dynamic-${q}`, name: userPart, email: q, isExternal: true });
      }
    } else {
      const cobranceEmail = `${q}@cobrance.com.br`;
      if (!seen.has(cobranceEmail)) {
        unique.unshift({ id: `cobrance-${q}`, name: q, email: cobranceEmail, isExternal: true });
      }
    }
  }
...
async function addRecipient(tabId, contact) {
  ...
  if (!exists) {
    ...
    await browser.compose.setComposeDetails(tabId, { [mode]: current });
    await saveRecentContact(contact);
  }
}

// ========== NOVO: removeRecipient via API ==========
async function removeRecipient(tabId, email) {
  if (!tabId || !email) return;
  const settings = await getSettings();
  const mode = settings.recipientMode || "to";
  try {
    const details = await browser.compose.getComposeDetails(tabId);
    ...
    const updated = current.filter((r) => {
      ...
      // Extrai o email se vier no formato "Nome <email>"
      const match = recipientEmail.match(/<(.+)>/);
      const cleanEmail = match ? match[1] : recipientEmail;
      return cleanEmail.toLowerCase() !== email.toLowerCase();
    });
    await browser.compose.setComposeDetails(tabId, { [mode]: updated });
  } catch (e) {
    console.error("AutoMention removeRecipient error:", e);
  }
}

browser.runtime.onMessage.addListener((message, sender) => {
  if (message.type === "automention-get-settings") {
    return getSettings().then((settings) => ({ ok: true, settings, licenseValid: true }));
  }
  ...
  if (message.type === "automention-add-recipient") {
    ...
    if (!tabId) return Promise.resolve({ ok: false, error: "No compose tab detected." });
  ...
  // ========== NOVO: handler para remover destinatário ==========
  if (message.type === "automention-remove-recipient") {
    const tabId = sender?.tab?.id;
    if (!tabId) return Promise.resolve({ ok: false, error: "No compose tab detected." });
    return removeRecipient(tabId, message.email)
      .then(() => ({ ok: true }))
      .catch((error) => {
        console.error("AutoMention remove recipient error:", error);
        return { ok: false, error: error?.message || String(error) };
      });
  }
...
browser.compose.onBeforeSend.addListener(async (tab, details) => {
  try {
    await browser.tabs.sendMessage(tab.id, { type: "automention-clean-ui" });
  } catch (e) {
